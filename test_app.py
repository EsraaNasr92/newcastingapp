...
class CastingAgencyTestCase(unittest.TestCase):
  ...
  def test_create_actors (self):
    res = self.client().post('/actors', json=self.new_actor, headers={"Authorization": (Casting_Director)})
    data = json.loads(res.data)
  ...
  def test_update_movies (self):
    ...
    res = self.client().patch('/movies/22', json=self.update_movie, headers={"Authorization": (Casting_Director)})
    data = json.loads(res.data)
  ...
  def test_delete_actors (self):
    self.client().post('/actors', json=self.new_actor, headers={"Authorization": (Casting_Director)})
  ...
  def test_delete_movies (self):
    self.client().post('/movies', json=self.new_movie, headers={"Authorization": (Executive_Producer)})
